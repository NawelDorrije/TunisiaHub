...
public class EmailService {
    ...
    public void sendReservationConfirmation(ReservationRestaurant reservation) {
        try {
            ...
            if (!StringUtils.hasText(recipient)) {
                log.warn("Skipping reservation confirmation email for reservation {} because user email is missing",
    ...
    public void sendConfirmationReservation(ReservationActivite reservation) {
        try {
            ...
            mailSender.send(message);
            log.info("Confirmation email sent to: {}", reservation.getUser().getEmail());
        } catch (Exception e) {
    ...
    public void sendStatutUpdate(ReservationActivite reservation) {
        try {
            ...
            mailSender.send(message);
            log.info("Status update email sent to: {}", reservation.getUser().getEmail());
        } catch (Exception e) {
    ...
    public void sendConfirmationPaiement(ReservationActivite reservation) {
        try {
            ...
            mailSender.send(message);
            log.info("Payment email sent to: {}", reservation.getUser().getEmail());
        } catch (Exception e) {
            log.error("Failed to send payment email: {}", e.getMessage(), e);
        }
    }
    ...
    public void sendConfirmationPaiementTotal(ReservationActivite reservation) {
        try {
            ...
            mailSender.send(message);
            log.info("Full payment email sent to: {}", reservation.getUser().getEmail());
        } catch (Exception e) {
            log.error("Failed to send full payment email: {}", e.getMessage(), e);
        }
    }
    ...
    public void sendConfirmationPaiementTranche(ReservationActivite reservation, int numTranche) {
        try {
            ...
            mailSender.send(message);
            log.info("Installment {} email sent to: {}", numTranche, reservation.getUser().getEmail());
        } catch (Exception e) {
            log.error("Failed to send installment email: {}", e.getMessage(), e);
        }
    }
    ...
    public void sendRappelEvenement(ReservationActivite reservation) {
        try {
            ...
            mailSender.send(message);
            log.info("Reminder email sent to: {}", reservation.getUser().getEmail());
        } catch (Exception e) {
    ...
    private String buildRappelHtml(ReservationActivite r) {
        ...
        String dateEvt = r.getActivite().getDateEvenement() != null
                ? sdf.format(r.getActivite().getDateEvenement()) : "Date non definie";
        ...
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8f9fa;padding:20px;">
          ...
          <div style="background:white;padding:30px;border-radius:0 0 12px 12px;">
            <p style="color:#495057;font-size:16px;">Bonjour <strong>%s %s</strong>,</p>
            <p style="color:#6c757d;">Votre activite est prevue <strong>%s</strong>.</p>
